#!/usr/bin/env python3
"""
Process extracted PDF text and split into chapters
"""
import re
import os
...
def find_chapter_boundaries(lines):
    ...
    for i, line in enumerate(lines):
        ...
        if re.match(r'^CHAPTER\s+\d+', line, re.IGNORECASE):
            ...
            current_start = i
        elif re.match(r'^Foreword', line, re.IGNORECASE):
            if current_chapter:
                boundaries.append((current_chapter, current_start, i))
            current_chapter = "foreword"
            current_start = i
        elif re.match(r'^Preface', line, re.IGNORECASE):
            if current_chapter:
                boundaries.append((current_chapter, current_start, i))
            current_chapter = "preface"
            current_start = i
        elif re.match(r'^In\s+Closing', line, re.IGNORECASE):
            if current_chapter:
                boundaries.append((current_chapter, current_start, i))
...
def extract_chapter_content(lines, start, end):
    ...
    content = re.sub(r'\s*\d+\s+Front Line PHP\s*', '', content, flags=re.MULTILINE)
...
def main():
    ...
    for chapter_key, start, end in boundaries:
        content = extract_chapter_content(lines, start, end)
        ...
        if file_path and content:
            ...
            print(f"Processed: {file_path} ({end - start} lines)")
